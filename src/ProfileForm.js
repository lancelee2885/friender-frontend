...
function ProfileForm({ handleUpdate }) {
  ...
  //TODO: useEffect to update isLoading state to false so we can get currUser in context

  function checked(hobby) {

    if (formData.hobbies.includes(hobby)) {
      return (
        <>
          <input
            type="checkbox" id="hobbies" name="hobbies" value={hobby} onChange={handleChange} checked ></input>
      ...
      return (
        <>
          <input
            type="checkbox" id="hobbies" name="hobbies" value={hobby} onChange={handleChange} ></input>
          <label htmlFor={hobby}>{hobby}</label>
        </>
      )
    }
  ...
  function handleChange(evt) {
    ...
    const updatehobbies = Array.from(document.querySelectorAll("input[name=hobbies]:checked")).map(i => i.value);
